...
describe("PipelineRunner", () => {
    beforeEach(() => {
        ...
        vi.spyOn(LengthNormalizerAgent.prototype, "normalizeChapter").mockImplementation(async ({ chapterContent, lengthSpec }) => ({
    ...
    it("uses a friendly fallback warning when sqlite memory indexing is unavailable", async () => {
        ...
        try {
            ...
            expect(warnings).toContain("当前 Node 运行时不支持 SQLite 记忆索引，继续使用 Markdown 回退方案。");
    ...
    it("normalizes revised output once before re-audit when it leaves the target band", async () => {
        ...
        const normalizeChapter = vi.mocked(LengthNormalizerAgent.prototype.normalizeChapter).mockResolvedValue({
    ...
    it("records a length warning when a single normalize pass still misses the hard range", async () => {
        ...
        try {
            ...
            expect(normalizeChapter).toHaveBeenCalledTimes(1);
            expect(result.lengthWarnings?.[0]).toContain("超出硬区间");
            expect(result.lengthTelemetry?.finalCount).toBe(stillOverHard.length);
