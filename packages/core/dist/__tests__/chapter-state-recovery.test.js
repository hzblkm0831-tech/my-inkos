...
import { buildStateDegradedPersistenceOutput, buildStateDegradedReviewNote, parseStateDegradedReviewNote, resolveStateDegradedBaseStatus, retrySettlementAfterValidationFailure, } from "../pipeline/chapter-state-recovery.js";
function createBook() {
    return {
        id: "test-book",
        title: "Test Book",
...
function createValidationWarning(overrides = {}) {
    return {
...
function createWriteChapterOutput(overrides = {}) {
    return {
        chapterNumber: 3,
        title: "第三章",
        content: "铜牌贴在胸口。",
...
function createChapterMeta(overrides = {}) {
    return {
        number: 3,
        title: "第三章",
...
describe("chapter-state-recovery", () => {
    it("retries settlement with localized validation feedback and recovers on a clean retry", async () => {
        ...
        const writer = {
            settleChapterState: vi.fn(async (input) => {
...
//# sourceMappingURL=chapter-state-recovery.test.js.map