...
import { HOOK_ACTIVITY_THRESHOLDS, HOOK_AGENDA_LIMITS, HOOK_HEALTH_DEFAULTS, HOOK_PHASE_THRESHOLDS, HOOK_PRESSURE_WEIGHTS, HOOK_TIMING_PROFILES, resolveHookVisibilityWindow, } from "../utils/hook-policy.js";
describe("hook-policy", () => {
    it("exposes shared lifecycle timing profiles from a single policy surface", () => {
        ...
        expect(HOOK_TIMING_PROFILES.endgame.overdueAge).toBeGreaterThan(HOOK_TIMING_PROFILES["near-term"].overdueAge);
    });
    ...
    it("keeps lifecycle activity and refresh thresholds on the same policy surface", () => {
        ...
        expect(HOOK_ACTIVITY_THRESHOLDS.longArcQuietHoldMaxAge).toBeGreaterThan(HOOK_ACTIVITY_THRESHOLDS.freshPromiseAge);
        expect(HOOK_ACTIVITY_THRESHOLDS.refreshDormancy).toBeGreaterThan(HOOK_ACTIVITY_THRESHOLDS.longArcQuietHoldMaxDormancy);
    });
});
//# sourceMappingURL=hook-policy.test.js.map