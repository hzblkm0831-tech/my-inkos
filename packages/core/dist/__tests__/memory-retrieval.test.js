...
describe("retrieveMemorySelection", () => {
    ...
    it("extracts mentor-focused query terms without pulling guild-route negatives into English retrieval", () => {
        const extractQueryTerms = memoryRetrieval.extractQueryTerms;
        expect(extractQueryTerms).toBeDefined();
        const terms = extractQueryTerms?.("Pull focus back to the mentor debt and do not open a new frontier in this chapter.", "Handle guild noise without letting the guild route overtake the mentor-debt mainline.", ["Lin Yue does not abandon the mentor debt."]) ?? [];
    ...
    it("extracts 师债-focused query terms without pulling 商会路线 negatives into Chinese retrieval", () => {
        const extractQueryTerms = memoryRetrieval.extractQueryTerms;
        expect(extractQueryTerms).toBeDefined();
        const terms = extractQueryTerms?.("第51章把注意力拉回师债，不让商会路线盖过主线。", "处理商会噪音，但不允许商会路线盖过师债主线。", ["林月不会放弃师债。"]) ?? [];
