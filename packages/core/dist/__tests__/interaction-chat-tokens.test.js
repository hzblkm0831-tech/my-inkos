...
describe("chat tool – maxTokens forwarding", () => {
    ...
    it("does not pass maxTokens to chatCompletion when depth has no maxTokens set", async () => {
        const tools = createInteractionToolsFromDeps(fakePipeline, fakeState, {
        ...
        const options = mockChatCompletion.mock.calls[0]?.[3];
    ...
    it("rethrows real chatCompletion errors instead of silently falling back", async () => {
        ...
        await expect(tools.chat?.("你好", { bookId: "test-book", automationMode: "manual" })).rejects.toThrow("provider down");
    });
});
...
//# sourceMappingURL=interaction-chat-tokens.test.js.map