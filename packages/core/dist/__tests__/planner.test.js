...
describe("PlannerAgent", () => {
    ...
    it("prefers a matched outline node over ordinary current focus text", async () => {
        await Promise.all([
            writeFile(join(storyDir, "current_focus.md"), [
                "# Current Focus",
                "",
                "## Active Focus",
                "",
    ...
    it("derives structured current_focus markdown into goal, avoids, and style emphasis", async () => {
        ...
        expect(result.intent.mustAvoid).not.toContain("Keep the chapter centered on a missing record, not a whole-conspiracy overview.");
