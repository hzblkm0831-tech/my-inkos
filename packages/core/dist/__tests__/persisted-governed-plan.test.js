...
describe("persisted governed plan helpers", () => {
    ...
    it("rejects persisted intents whose goal is still a placeholder", async () => {
        ...
        const runtimeDir = join(bookDir, "story", "runtime");
        await mkdir(runtimeDir, { recursive: true });
        await writeFile(join(runtimeDir, "chapter-0003.intent.md"), [
            "# Chapter Intent",
            "",
            "## Goal",
    ...
    it("normalizes persisted artifact paths relative to the book directory", () => {
        expect(relativeToBookDir("/tmp/book", "/tmp/book/story/runtime/chapter-0001.intent.md")).toBe("story/runtime/chapter-0001.intent.md");
    });
});
