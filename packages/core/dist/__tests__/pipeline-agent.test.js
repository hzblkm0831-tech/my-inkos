...
import { PipelineRunner, StateManager } from "../index.js";
describe("agent pipeline tools", () => {
    ...
    it("plans and composes chapters through the agent tool surface", async () => {
        const planResult = JSON.parse(await executeAgentTool(pipeline, state, config, "plan_chapter", { bookId, guidance: "Ignore the guild chase and focus on the mentor conflict." }));
        expect(planResult.intentPath).toBe("story/runtime/chapter-0001.intent.md");
        const composeResult = JSON.parse(await executeAgentTool(pipeline, state, config, "compose_chapter", { bookId, guidance: "Ignore the guild chase and focus on the mentor conflict." }));
    ...
    it("keeps update_current_focus usable for explicit local overrides through the tool surface", async () => {
        ...
        const planResult = JSON.parse(await executeAgentTool(pipeline, state, config, "plan_chapter", { bookId }));
    ...
    it("blocks write_full_pipeline when runtime progress is ahead of the chapter index", async () => {
        ...
        const result = JSON.parse(await executeAgentTool(pipeline, state, config, "write_full_pipeline", { bookId, count: 1 }));
    ...
    it("blocks write_truth_file from hacking chapter progress inside current_state.md", async () => {
        const result = JSON.parse(await executeAgentTool(pipeline, state, config, "write_truth_file", {
            bookId,
...
//# sourceMappingURL=pipeline-agent.test.js.map