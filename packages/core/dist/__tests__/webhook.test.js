...
describe("sendWebhook", () => {
    ...
    it("sends event when it matches subscribed events list", async () => {
        mockFetch.mockResolvedValueOnce({ ok: true });
        await sendWebhook({ url: "https://example.com/hook", events: ["chapter-complete"] }, basePayload);
        expect(mockFetch).toHaveBeenCalledOnce();
    });
    ...
    it("throws on non-ok response", async () => {
        ...
        await expect(sendWebhook({ url: "https://example.com/hook" }, basePayload)).rejects.toThrow("Webhook POST to https://example.com/hook failed: 500");
    });
});
