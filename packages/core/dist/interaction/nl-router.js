export function routeNaturalLanguageIntent(input, context = {}) {
    ...
    if (modeCommand) {
        return {
            intent: "switch_mode",
            mode: modeCommand[1].toLowerCase(),
        };
    }
    ...
    if (slashExport) {
        return {
            intent: "export_book",
            ...(bookId ? { bookId } : {}),
            format: slashExport[1]?.toLowerCase() ?? "txt",
        };
    }
    ...
    if (/^(导出全书(?:为\s*(epub|md|txt))?|export book(?: as)?\s*(epub|md|txt)?)$/i.test(trimmed)) {
        const matchedFormat = trimmed.match(/(epub|md|txt)/i)?.[1]?.toLowerCase();
        return {
            intent: "export_book",
            ...(bookId ? { bookId } : {}),
