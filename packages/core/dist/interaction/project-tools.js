...
import { BookCreationDraftSchema } from "./session.js";
...
function parseCreationDraftResult(text) {
...
export function createInteractionToolsFromDeps(pipeline, state, hooks) {
    ...
    return {
        ...
        developBookDraft: async (input, existingDraft) => {
            ...
            const response = await chatCompletion(instrumentedPipeline.config.client, instrumentedPipeline.config.model, [
                {
                    role: "system",
                    content: [
        ...
        chat: async (input, options) => {
            ...
            if (instrumentedPipeline.config?.client && instrumentedPipeline.config?.model) {
                try {
                    response = await chatCompletion(instrumentedPipeline.config.client, instrumentedPipeline.config.model, [
                        {
                            role: "system",
                            content: [
