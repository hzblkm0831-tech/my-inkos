...
export async function executeAgentTool(pipeline, state, config, name, args) {
    switch (name) {
        case "plan_chapter": {
            const result = await pipeline.planChapter(args.bookId, args.guidance);
            return JSON.stringify(result);
        }
        case "compose_chapter": {
            const result = await pipeline.composeChapter(args.bookId, args.guidance);
            return JSON.stringify(result);
        }
        case "write_draft": {
            ...
            const result = await pipeline.writeDraft(bookId, args.guidance);
            return JSON.stringify(result);
        }
        case "audit_chapter": {
        ...
        case "revise_chapter": {
            ...
            const bookId = args.bookId;
            const chapterNum = args.chapterNumber;
            ...
            const result = await pipeline.reviseDraft(bookId, chapterNum, args.mode ?? DEFAULT_REVISE_MODE);
            return JSON.stringify(result);
        }
        case "scan_market": {
        ...
        case "update_author_intent": {
            await state.ensureControlDocuments(args.bookId);
            const { writeFile } = await import("node:fs/promises");
            const { join } = await import("node:path");
            const storyDir = join(state.bookDir(args.bookId), "story");
            await writeFile(join(storyDir, "author_intent.md"), args.content, "utf-8");
        ...
        case "update_current_focus": {
            ...
            const { join } = await import("node:path");
            const storyDir = join(state.bookDir(args.bookId), "story");
            await writeFile(join(storyDir, "current_focus.md"), args.content, "utf-8");
...
async function getSequentialWriteGuardError(state, bookId, toolName) {
...
function containsProgressManipulation(content) {
    const patterns = [
