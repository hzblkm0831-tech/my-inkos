...
function fallbackExtractContent(raw, countingMode) {
    ...
    const proseLines = lines.filter((line) => {
        ...
        if (/^===\s*[A-Z_]+\s*===/.test(trimmed))
            return false;
        if (/^(PRE_WRITE_CHECK|CHAPTER_TITLE|章节标题|写作自检)[：:]/.test(trimmed))
            return false;
        return true;
    });
