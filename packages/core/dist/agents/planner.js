...
import { parseChapterSummariesMarkdown, renderHookSnapshot, renderSummarySnapshot, retrieveMemorySelection, } from "../utils/memory-retrieval.js";
...
export class PlannerAgent extends BaseAgent {
    ...
    async planChapter(input) {
        ...
        const [authorIntent, currentFocus, storyBible, volumeOutline, chapterSummaries, bookRulesRaw, currentState,] = await Promise.all([
        ...
        const intentMarkdown = this.renderIntentMarkdown(intent, input.book.language ?? "zh", renderHookSnapshot(memorySelection.hooks, input.book.language ?? "zh"), renderSummarySnapshot(memorySelection.summaries, input.book.language ?? "zh"), activeHookCount);
    ...
    buildStructuredDirectives(input) {
        ...
        return {
            arcDirective: this.buildArcDirective(input.language, input.volumeOutline, input.outlineNode, input.matchedOutlineAnchor),
    ...
    collectConflicts(externalContext, currentFocus, outlineNode, volumeOutline) {
        const outlineText = outlineNode ?? volumeOutline;
        if (!outlineText || outlineText === "(文件尚未创建)")
            return [];
        if (externalContext) {
            const indicatesOverride = /ignore|skip|defer|instead|不要|别|先别|暂停/i.test(externalContext);
            if (!indicatesOverride && this.hasKeywordOverlap(externalContext, outlineText))
                return [];
            return [
                {
    ...
    buildArcDirective(language, volumeOutline, outlineNode, matchedOutlineAnchor) {
    ...
    buildSceneDirective(language, cadence) {
    ...
    buildMoodDirective(language, cadence) {
    ...
    buildTitleDirective(language, cadence) {
    ...
    hasMatchedOutlineAnchor(volumeOutline, chapterNumber) {
        const lines = volumeOutline.split("\n").map((line) => line.trim()).filter(Boolean);
        return lines.some((line) => this.matchExactOutlineLine(line, chapterNumber) !== undefined
    ...
    renderIntentMarkdown(intent, language, pendingHooks, chapterSummaries, activeHookCount) {
